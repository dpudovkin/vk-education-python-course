...
class Order(models.Model):
    destination_client = models.ForeignKey(to=Client, null=False, on_delete=models.PROTECT,
                                           related_name='received_orders',
                                           verbose_name='Клиент получающий отправление')
    arriving_client = models.ForeignKey(to=Client, null=False, on_delete=models.PROTECT,
                                        related_name='sent_orders',
                                        verbose_name='Клиент отдающий отправление')
    destination_address = models.ForeignKey(to=Address, on_delete=models.PROTECT,
                                            related_name='received_orders',
                                            verbose_name='Адрес назначения заказа', null=False)
    arriving_address = models.ForeignKey(to=Address, on_delete=models.PROTECT,
                                         related_name='sent_orders',
    ...
    performer = models.ForeignKey(to=Employee, null=False, on_delete=models.PROTECT,
                                  verbose_name='Курьер')
